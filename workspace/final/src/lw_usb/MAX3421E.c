...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write val via SPI
	status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, 2);
	if(status != 0) {
		xil_printf("Error writing val via SPI\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, nbytes+1);
	if(status != 0) {
		xil_printf("Error writing data via SPI\n");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, 2);
	if(status != 0) {
		xil_printf("Error reading val via SPI\n");
	}
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, nbytes+1);
	if(status != 0) {
		xil_printf("Error reading data via SPI\n");
	}
...
void MAX3421E_reset(void) {
	...
	xil_printf ("Releasing USB from Reset\n");
	BYTE revision = MAXreg_rd(rREVISION);
	xil_printf("Read revision register value: %d\n", revision);
